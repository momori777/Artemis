...
if not _COMFYUI_ROOT:
    # Use git-downloaded ComfyUI from deps directory
    _COMFYUI_ROOT = os.path.join(_def, 'deps', 'ComfyUI')
...
if __name__ == "__main__":
    ...
    if not no_manage_llama and not should_stop_llama("comfyui"):
    ...
    # 注册清理钩子（使用 shared 模块）——锁清理始终注册；llama 重启保护仅在管理模式下启用

...
import torchaudio
# torchaudio >= 2.9 removed set_audio_backend / sox backend; torchcodec needs
# FFmpeg DLLs which are often missing. Patch torchaudio.load to use soundfile
# (pure Python + libsndfile, no FFmpeg) so ref-audio loading works on CPU.
if not hasattr(torchaudio, 'set_audio_backend'):
    import soundfile as _sf
    import numpy as _np
    _orig_load = torchaudio.load
    def _sf_load(path, out=None, normalization=True, channels_first=True):
        import torch as _t
        data, sr = _sf.read(path, dtype='float32', always_2d=True)
        t = _t.from_numpy(data)  # (frames, channels)
        if channels_first:
            t = t.t()  # (channels, frames)
        if out is not None:
            out.copy_(t)
            return out, sr
        return t, sr
...
class TTS_Config:
    ...
    def save_configs(self, configs_path: str = None) -> None:
        ...
        if self.configs is not None:
            ...
            # Portability: rewrite absolute weight paths (under the engine root)
            # as relative paths so the yaml stays machine-independent.
            try:
                root = os.getcwd()
                for k in ("t2s_weights_path", "vits_weights_path"):
                    v = configs["custom"].get(k, "")
                    if isinstance(v, str) and os.path.isabs(v):
                        rp = os.path.relpath(v, root)
                        if not rp.startswith(".." + os.sep):
                            configs["custom"][k] = rp.replace(os.sep, "/")
            except Exception:
                pass
...
class TTS:
    ...
    def init_cnhuhbert_weights(self, base_path: str):
        ...
        elif not self.configs.is_half:
            # CPU/float32: hubert ckpt is fp16; force float32 to match inputs
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        if self.configs.is_half and str(self.configs.device) != "cpu":
            ...
            # CPU/float32 mode: some v2Pro ckpts store fp16 weights; force float32
            # so inference inputs (float32) match the weight dtype.

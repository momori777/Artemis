...
# Lazy-load BERT (only needed for Chinese; Japanese inference skips this)
tokenizer = None
bert_model = None
def _ensure_bert():
    ...
    if tokenizer is not None and bert_model is not None:
        return
    if not os.path.exists(bert_path) and not bert_path.startswith(("http", "hf://")):
        print(f"[INFO] BERT model not found at {bert_path}, Chinese G2P won't work", file=sys.stderr)
        tokenizer = False  # sentinel
        bert_model = False
        return
    try:
        ...
        print(f"[WARN] BERT load failed: {e}", file=sys.stderr)
        tokenizer = False
        bert_model = False
...
# Lazy-load speaker encoder (cnhubert, needed for speaker embedding from ref audio)
ssl_model = None
def _ensure_ssl():
    global ssl_model
    ...
    try:
        ...
        print(f"[WARN] CNHubert load failed: {e}", file=sys.stderr)
        ssl_model = False
...
# Module-level weight loading moved to _ensure_weights() (lazy)
...
# Defer weight loading: gpt_path / sovits_path are set by caller (tts_call.py)
# via environment variables before get_tts_wav() is called.
# Module-level weight loading is NOT done here to avoid import-time failures.
_weights_loaded = False
def _ensure_weights():
    global _weights_loaded, hps
    if _weights_loaded:
        return
    if gpt_path and os.path.exists(gpt_path):
        change_gpt_weights(gpt_path)
    else:
        print(f"[WARN] gpt_path not set or not found: {gpt_path}", file=sys.stderr)
    if sovits_path and os.path.exists(sovits_path):
        try:
            next(change_sovits_weights(sovits_path))
            print(f"[WEIGHTS] SoVITS loaded from {sovits_path}", file=sys.stderr)
        except Exception as e:
            print(f"[WARN] SoVITS weight load failed: {e}", file=sys.stderr)
    else:
        print(f"[WARN] sovits_path not set or not found: {sovits_path}", file=sys.stderr)

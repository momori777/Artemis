#!/usr/bin/env python3
"""
ComfyUI 文生图调用脚本 (AMD GPU 适配版)
直接加载模型推理，不走 WebUI HTTP API。

与 NVIDIA 版的区别:
  - torch.cuda.empty_cache() → safe_empty_cache() GPU 无关封装
  - 不改变核心推理逻辑 (仍依赖 ComfyUI 的 torch 设备分配)
  - AMD 用户需确保 ComfyUI 安装了 DirectML/ZLUDA 版 torch
...
可用模型:
  WAI-Nsfw-Illustrious-17.safetensors  (6.5GB)
  miaomiaoHarem_v20.safetensors        (6.5GB)
"""
import sys
import os
import time
...
# --- shared 生命周期模块 (AMD 适配版已在 AMD_GPU/ 中覆盖) ---
...
def safe_empty_cache():
    """
    GPU 无关的显存清理。依次尝试 CUDA/HIP → 通用 gc。
    AMD 上 ROCm torch 也映射到 torch.cuda，所以 CUDA 路径理论上也能走。
    """
    import gc
    try:
        import torch
...
def safe_load_safetensors(ckpt_path):
    """
    手动加载 safetensors 文件。
    绕过 cu130 在 RTX 5070 上的 bug，同时对 AMD GPU 也无害。
    """
    import torch

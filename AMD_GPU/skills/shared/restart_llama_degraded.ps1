...
#  - 去掉 -ctk q8_0 / -ctv q8_0
#  - ngl 固定 99 (Vulkan 全部层 offload)
#>
param(
  [int]$ForceBatch = -1
)
...
# ========== 主流程 ==========
Write-Host "=== Llama Degraded Restart (Vulkan, ngl=99, batch downscaling) ===" -ForegroundColor Cyan
Write-Host "Exe:   $exe"
...
if ($started) {
  Write-Host ""
  Write-Host "=== SUCCESS: ngl=99 (Vulkan), batch=$finalBatch/$finalUbatch, port=$port ===" -ForegroundColor Green
  # 重启 watchdog

...
**[Silicon Rider Bench](https://github.com/kcores/silicon-rider-bench)** はエージェントベンチマーク：仮想の配達ライダーをシミュレートし、ナビゲート、注文受領、取餐、配達、バッテリー管理で 24 時間シミュレーションの総利益スコアリングを行う。全ラウンドで同じシード（**622539**）を使用。

これは**純黒箱エージェントテスト：モデルはツール呼び出しのみで自身で判断（検索 → 受領 → ルート計画 → 移動 → 取餐 → 配達 → バッテリー交換）、外部支援なし。プロンプトには 2 条の規則：既に計算したルート（ルート再利用）、**バッテリー < 30% で充電ルート計画必須**。
...
| 指標 | deepseek-v4-flash<br>(無限 ctx) | Hermes 35B MoE<br>(25 ctx) | **Hermes 35B MoE<br>(100 ctx)** ✅ |
|-|-|-|-|
| **総利潤 ¥** | **619.6** | 411.3 | **524.6** |
| 完了注文 | **33** | 30 | 28 |
| **定時率** | **81.8%** | 56.7% | **75.0%** |
| ルート効率 | **1.34** | 1.77 | 1.68 |
| API 違反率 | **1.3%** | 2.3% | 2.2% |
| 1件あたり利潤 ¥ | **18.77** | 13.71 | **18.74** |
| 遅延罰金 ¥ | **2.75** | 107.9 | 44.7 |
| 総トークン | 24.39M | 1.35M | 4.08M |
| トークン効率 ¥/M | 25.4 | 304.6 | **128.6** |

- **コンテキスト長が第一のレバー**：`CONTEXT_HISTORY_LIMIT` を 25 から 100 に上げることで、定時率が **56.7% → 75%** に向上、遅延罰金が **¥107.9 → ¥44.7** に減少、利潤が **¥411 → ¥525** に増加（モデルがついにターンを跨いで注文締切とルートを保持できるようになった）。
- **ローカル 35B MoE ≈ クラウド flash の 85%**：100 ctx でローカル量子化 35B が **¥524.6 = dsv4-flash ¥619.6 の 84.6%** に到達。定時率（75% vs 81.8%）と 1 件あたり利潤（¥18.74 vs ¥18.77）はほぼ同等。
- **コストはわずか 1/6**：flash は **24.39M トークン**（無限コンテキスト）を消費；ローカル 100 ctx は **4.08M** のみで利潤の 5/6 を獲得 → **トークン効率が 5 倍**、かつゼロ API コスト。
- **残りの差**：ルート効率（1.68 vs 1.34）——35B-A3B の約 3B 活性化パラメータは、複数区間の最適ルート計画で flash に劣る。

結論：Hermes3.6 量化精修後 qwen3.6 35B の agentic 能力は基本として claude opus 4.6 に相当！

> 🧪 完全ログとレポートは `docs/silicon-rider-bench-622539/`（COMPARISON-622539.md および各ラウンドサマリー）参照。
...
- スタートアップ時にサマリーからコンテキストを復元し、実際のトークン数を 5K-20K 範囲に保持

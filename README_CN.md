...
**[Silicon Rider Bench](https://github.com/kcores/silicon-rider-bench)** 是一个智能体基准测试：模拟外卖骑手在虚拟城市中导航、接单、取餐、准时送达并管理电量，以模拟 24 小时内的总利润评分。所有轮次使用相同种子（**622539**）以保证可比性。

这是**纯黑盒智能体测试**：模型完全靠自己通过工具调用做决策（搜索 → 接单 → 规划路线 → 移动 → 取餐 → 送达 → 换电），无任何外部辅助。提示词包含两条规则：记忆已算过的路线（路线复用）、**电量 < 30% 时必须规划充电路径**。
...
- **成本仅 1/6**：flash 吃了 **24.39M token**（无限上下文）；本地 100 ctx 只用 **4.08M** 就拿下 5/6 的利润 → **token 效率高 5 倍**，且零 API 成本。
- **剩余差距**：路径效率（1.68 vs 1.34）——35B-A3B 仅 ~3B 活跃参数，在多程最优路线规划上仍弱于 flash。

结论：Hermes3.6量化精修后qwen3.6 35B agentic能力基本等同claude opus 4.6!

> 🧪 完整日志与报告见 `docs/silicon-rider-bench-622539/`（COMPARISON-622539.md 及各轮摘要）。
...
- **必须为 local llama 模型禁用 `cron` 工具** -- llama.cpp 的 GBNF 语法转换器拒绝任何没有用 `^...$` 完整锚定的 JSON Schema `pattern`。OpenClaw 的 cron 工具在嵌套属性(`job.declarationKey`、`job.displayName`)上声明了 `pattern: "\\S"`,因此**每一个**携带完整工具集的请求都会返回 `400 JSON schema conversion failed: Pattern must start with '^' and end with '$'`,然后静默 fallback 到远程模型。只有嵌套/数组层级的 pattern 会触发,顶层的可以正常转换。在 `~/.openclaw/openclaw.json` 中修复:
  ```jsonc
  "tools": {
    "byProvider": {
  ...
  ```
  `tools.*` 不支持热重载,改完需要重启 gateway。定时任务仍可通过远程模型的 session 创建。
